...
{
    public class BidYJYXJS : WebSiteCrawller
    {
        public BidYJYXJS()
            : base()
        {
            this.Group = "中标信息";
            this.Title = "广东省阳江市阳西县建设工程中标信息";
            this.Description = "自动抓取广东省阳江市阳西县建设工程中标信息";
            this.PlanTime = "9:30,11:30,14:30,16:30,18:30";
            this.SiteUrl = "http://www.yjgcjy.cn/BeingList.aspx?SiteId=23";
        }
                    ...
                    for (int j = 1; j < table.RowCount - 1; j++)
                    {
                        ...
                        if (dtnode != null && dtnode.Count > 0)
                        {
                            ...
                            if (tabledetail != null && tabledetail.RowCount > 0)
                            {
                                ...
                                buildUnit = bildUnit.Match(bidCtx).Value.Replace("招标人：", "").Replace("：", "").Trim();
                                if (buildUnit == "")
                                {
                                    buildUnit = "";
                                }
                                Regex regMoney = new Regex(@"(中标价|发包价)：[^\r\n]+\r\n");
                                bidMoney = regMoney.Match(bidCtx).Value.Replace("中标价：", "").Trim();
                                Regex regBidMoney = new Regex(@"[0-9]+[.]{0,1}[0-9]+");
                                if (bidMoney.Contains("总投资："))
                                {
                                    Regex regMoneytwo = new Regex(@"总投资：[0-9]+[.]{0,2}[0-9][0-9]万元");
                                    bidMoney = regMoneytwo.Match(bidMoney).ToString().Replace("总投资：", "").Trim();
                                }
                                ...
                                //bidCtx = dtnode.ToHtml();
                                bidType = ToolHtml.GetInviteTypes(prjName);
                                BidInfo info = ToolDb.GenBidInfo("广东省", "阳江市区", "阳西县", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType,
