...
namespace Crawler.Instance
{
    ...
    /// 广东省惠州市惠东县
    /// </summary>
    public class BidHuiZhouHDX : WebSiteCrawller
    {
        public BidHuiZhouHDX()
            : base()
        {
            this.Group = "中标信息";
            this.Title = "广东省惠州市惠东县";
            this.PlanTime = "01:08,9:08,10:08,11:38,14:08,15:08,17:38";
            this.Description = "自动抓取广东省惠州市惠东县中标信息";
            this.ExistCompareFields = "Prov,City,Area,Road,Code,ProjectName,InfoUrl";
            this.SiteUrl = "http://www.ebc.huizhou.gov.cn/index/showList/000000000004/000000000428";
        }
            ...
            if (aNodes != null && aNodes.Count > 0)
            {
                ...
                for (int t = 1; t < table.RowCount - 1; t++)
                {
                    ...
                    if (ctxNode != null && ctxNode.Count > 0)
                    {
                        ...
                        Regex regexBuildUnit = new Regex("(中标人|中标单位|中 标 人)：[^\r\n]+[\r\n]{1}");
                        ...
                        Regex regexbidUnit = new Regex("(招标人|建设单位|招 标 人)：[^\r\n]+[\r\n]{1}");
                        Match matchbidUnit = regexbidUnit.Match(tabTag.ToPlainTextString());
                        if (matchbidUnit.Value.Length > 0)
                        {
                            bidUnit = matchbidUnit.Value.Replace("招 标 人：", "").Replace("招标人：", "").Replace("建设单位：", "").Replace("\r\n", "").Replace("\t", "").Replace("&nbsp;", " ").Trim();
                        }

                        Regex regexMoney = new Regex("(中标价|其中标价为|中标价格|中 标 价)：[^\r\n]+[\r\n]{1}");
                        Match matchMoney = regexMoney.Match(tabTag.ToPlainTextString());
                        if (matchMoney.Value.Length > 0)
                        {
                            bidMoney = matchMoney.Value.Replace("中 标 价：", "").Replace("中标价：", "").Replace("其中标价为：", "").Replace("中标价格：", "").Replace("\r", "");
                        }
                    ...
                    BidInfo info = ToolDb.GenBidInfo("广东省", "惠州市区", "惠东县", string.Empty, code, prjName, bidUnit, beginDate, buildUnit, beginDate, endDate, ctx, string.Empty, "惠州市建设工程交易中心",bidType, "建设工程", string.Empty, bidMoney, InfoUrl, string.Empty, HtmlTxt);
