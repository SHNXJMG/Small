...
{
    public class BidFoshanNH : WebSiteCrawller
    {
        public BidFoshanNH()
            : base()
        {
            ...
            this.Description = "自动抓取广东省佛山市南海区中标信息";
            this.SiteUrl = "http://www.fsggzy.cn/gcjy/gc_zbxx1/gczbgq/gc_1zbnh/";
        }
                ...
                if (sNode != null && sNode.Count > 0)
                {
                    for (int t = 0; t < sNode.Count; t++)
                    {
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            specType = "建设工程";
                            BidInfo info = ToolDb.GenBidInfo("广东省", "佛山市区", "南海区", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType, bidMoney, InfoUrl, prjMgr, HtmlTxt);
                            list.Add(info);
