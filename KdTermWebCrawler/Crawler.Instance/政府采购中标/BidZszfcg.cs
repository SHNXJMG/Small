...
{
    public class BidZszfcg : WebSiteCrawller
    {
        public BidZszfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "中山市政府采购中标信息";
            this.PlanTime = "9:19,11:19,13:49,15:49,17:49";
            this.Description = "自动抓取中山市政府采购中标信息";
            this.SiteUrl = "http://zhongshan.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://zhongshan.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://zhongshan.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (bidCtx.Contains("预算金额"))
                                bidMoney = "0";
                            bidType = prjName.GetInviteBidType();
                            msgType = "中山市政府采购";
                            specType = "政府采购";

                            BidInfo info = ToolDb.GenBidInfo("广东省", "中山市区", "", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType, bidMoney, InfoUrl, prjMgr, HtmlTxt);
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://zhaoqing.gdgpo.gov.cn" + fileAtag.Link;
