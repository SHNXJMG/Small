...
{
    public class BidYfzfcg : WebSiteCrawller
    {
        public BidYfzfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "云浮市政府采购中标信息";
            this.PlanTime = "9:00,10:00,13:00,15:00,17:00";
            this.Description = "自动抓取云浮市政府采购中标信息";
            this.SiteUrl = "http://yunfu.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://yunfu.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://yunfu.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (bidUnit.IndexOf("金额")>0)
                                bidUnit = bidUnit.Remove(bidUnit.IndexOf("金额"));
                            bidType = prjName.GetInviteBidType();
                            msgType = "云浮市政府采购";
                            specType = "政府采购";

                            BidInfo info = ToolDb.GenBidInfo("广东省", "云浮市区", "", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType, bidMoney, InfoUrl, prjMgr, HtmlTxt);
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://yunfu.gdgpo.gov.cn" + fileAtag.Link;
