...
{
    public class BidSgzfcg : WebSiteCrawller
    {
        public BidSgzfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "韶关市政府采购中标信息";
            this.PlanTime = "9:25,11:25,13:25,15:25,17:25";
            this.Description = "自动抓取韶关市政府采购中标信息";
            this.SiteUrl = "http://shaoguan.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://shaoguan.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://shaoguan.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (bidMoney == "0" || string.IsNullOrEmpty(bidMoney))
                            {
                                bidMoney = bidCtx.GetMoneyString(new string[]{"RMB"},false).GetMoney();
                            }
                            ...
                            BidInfo info = ToolDb.GenBidInfo("广东省", "韶关市区", "", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType, bidMoney, InfoUrl, prjMgr, HtmlTxt);
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://shaoguan.gdgpo.gov.cn" + fileAtag.Link;
