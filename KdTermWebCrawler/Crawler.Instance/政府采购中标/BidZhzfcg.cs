...
{
    public class BidZhzfcg : WebSiteCrawller
    {
        public BidZhzfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "珠海市政府采购中标信息";
            this.PlanTime = "9:17,11:17,13:47,15:47,17:47";
            this.Description = "自动抓取珠海市政府采购中标信息";
            this.SiteUrl = "http://zhuhai.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://zhuhai.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            BidInfo info = ToolDb.GenBidInfo("广东省", "珠海市区", "", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType, bidMoney, InfoUrl, prjMgr, HtmlTxt);
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://zhuhai.gdgpo.gov.cn" + fileAtag.Link;
