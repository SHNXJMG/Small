...
{
    public class BidZqzfcg : WebSiteCrawller
    {
        public BidZqzfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "肇庆市政府采购中标信息";
            this.PlanTime = "9:21,11:21,13:21,15:21,17:21";
            this.Description = "自动抓取肇庆市政府采购中标信息";
            this.SiteUrl = "http://zhaoqing.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://zhaoqing.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://zhaoqing.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (bidMoney == "0" || string.IsNullOrEmpty(bidMoney))
                            {
                               ...
                               if (bidMoney == "0" || string.IsNullOrEmpty(bidMoney))
                            {
                                bidMoney = bidCtx.GetMoneyString(new string[] { "￥","$" }, true).GetMoney();
                            }
