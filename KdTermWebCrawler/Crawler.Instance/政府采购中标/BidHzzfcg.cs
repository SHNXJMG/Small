...
{
    public class BidHzzfcg : WebSiteCrawller
    {
        public BidHzzfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "惠州市政府采购中标信息";
            this.PlanTime = "9:35,11:35,13:35,15:35,17:35";
            this.Description = "自动抓取惠州市政府采购中标信息";
            this.SiteUrl = "http://huizhou.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://huizhou.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://huizhou.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (code.Contains("的"))
                                code = code.Remove(code.IndexOf("的"));
                            if (code.Contains("中标"))
                                code = code.Remove(code.IndexOf("中标"));
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://huizhou.gdgpo.gov.cn" + fileAtag.Link;
