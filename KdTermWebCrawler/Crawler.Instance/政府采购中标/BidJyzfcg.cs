...
{
    public class BidJyzfcg : WebSiteCrawller
    {
        public BidJyzfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "揭阳市政府采购中标信息";
            this.PlanTime = "9:02,10:02,13:02,15:02,17:02";
            this.Description = "自动抓取揭阳市政府采购中标信息";
            this.SiteUrl = "http://jieyang.gdgpo.com/queryMoreInfoList/channelCode/0008.html";
            this.MaxCount = 50;
        }
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (code.Contains("于"))
                                code = code.Remove(code.IndexOf("于"));
                            bidType = prjName.GetInviteBidType();
                            msgType = "揭阳市政府采购";
                            specType = "政府采购";

                            BidInfo info = ToolDb.GenBidInfo("广东省", "揭阳市区", "", string.Empty, code, prjName, buildUnit, beginDate, bidUnit, beginDate, endDate, bidCtx, string.Empty, msgType, bidType, specType, otherType, bidMoney, InfoUrl, prjMgr, HtmlTxt);
