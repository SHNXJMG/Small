...
{
    public class BidSzLhFwZfcg : WebSiteCrawller
    {
        public BidSzLhFwZfcg()
            : base()
        {
            this.Group = "政府采购中标信息";
            this.Title = "深圳市龙华新区政府采购公共资源交易中心中标信息（服务）";
            this.Description = "自动抓取深圳市龙华新区政府采购公共资源交易中心中标信息（服务）";
            this.PlanTime = "9:22,13:53";
            this.SiteUrl = "http://lhxq.szzfcg.cn/portal/topicView.do?method=view1&id=500100204&siteId=11&tstmp=18%3A42%3A09%20GMT%2B0800&page=1";
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://lhxq.szzfcg.cn/portal/topicView.do?method=view1&id=500100204&siteId=11&tstmp=18%3A42%3A09%20GMT%2B0800&page=" + i, Encoding.UTF8);
                    }
                    catch { continue; }
                }
                            ...
                            if (aTagNode != null && aTagNode.Count > 0)
                            {
                                for (int k = 0; k < aTagNode.Count; k++)
                                {
                                    ...
                                    if (aFile.IsAtagAttach())
                                    {
                                        ...
                                        if (aFile.Link.Contains("http") || aFile.Link.ToLower().Contains("down"))
                                            link = aFile.Link;
                                        else
