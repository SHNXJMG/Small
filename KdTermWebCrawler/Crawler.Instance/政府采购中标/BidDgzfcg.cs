...
{
    public class BidDgzfcg : WebSiteCrawller
    {
        public BidDgzfcg()
            : base() 
        {
            this.Group = "政府采购中标信息";
            this.Title = "东莞市政府采购中标信息";
            this.PlanTime = "9:04,11:04,13:04,15:04,17:04";
            this.Description = "自动抓取东莞市政府采购中标信息";
            this.SiteUrl = "https://www.dgzb.com.cn/ggzy/website/WebPagesManagement/TradeInfo/GovProcurement/govlist?fcInfotype=7&openbidbelong=SS&govTypeIndex=2&belongIndex=0";
            this.MaxCount = 800;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            string c = b.Replace("('", "心情").Replace("')", "你猜");
            if (pageNode != null && pageNode.Count > 0)
            {
                try
                {
                    string temp = c.GetRegexBegEnd("心情", "你猜");
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i >= 1)
                {
                    ...
                    {
                        string a1 = nvc[0];
                        html = this.ToolWebSite.GetHtmlByUrl("https://www.dgzb.com.cn/ggzy/website/WebPagesManagement/TradeInfo/GovProcurement/findListByPage?fcInfotype=7&openbidbelong=SS", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                    ...
                    foreach (object arrValue in array)
                    {
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            if (bidUnitNode != null && bidUnitNode.Count > 0)
                            {
                                ...
                                try
                                {
                                    for (int s = 0; s < unitTable.Rows[0].ColumnCount; s++)
                                    {
                                        ctx += unitTable.Rows[0].Columns[s].ToNodePlainString() + "：";
                                        ctx += unitTable.Rows[1].Columns[s].ToNodePlainString() + "\r\n";
                                    }
                                }
                                catch { }
