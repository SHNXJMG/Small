...
namespace Crawler.Instance
{
    ...
    /// 内蒙古政府采购 自治区信息
    /// </summary>
    public class InviteZfNeimeng : WebSiteCrawller
    {
        public InviteZfNeimeng()
            : base()
        {
            this.Group = "政府采购招标信息";
            this.Title = "内蒙古政府采购自治区信息";
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                       string  SiteUrl1 = "http://www.nmgp.gov.cn/category/category-ajax.php?type_name=1&purorgform=1&byf_page=" + i + "&fun=cggg&_=1509440775608";
                ...
                //            prjName = alink.GetAttribute("title");
                ...
                //                prjAddress = inviteCtx.GetAddressRegex();
                //                if(string.IsNullOrEmpty(prjAddress))
                //                    prjAddress = inviteCtx.GetRegex("投标地点,开标地点");
                //                msgType = "内蒙古自治区政府采购中心";
                //                specType = "政府采购";
                //                inviteType = ToolHtml.GetInviteTypes(prjName);
                //                InviteInfo info = ToolDb.GenInviteInfo("内蒙古自治区", "内蒙古自治区及盟市", "", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, inviteType, specType, string.Empty, InfoUrl, HtmlTxt);
                //                list.Add(info);
                //                if (!crawlAll && list.Count >= this.MaxCount)
                //                    return list;
                //            }
