...
{
    public class InviteYjzfcg : WebSiteCrawller
    {
        public InviteYjzfcg()
            : base()
        {
            this.Group = "政府采购招标信息";
            this.Title = "阳江市政府采购招标信息";
            this.PlanTime = "9:10,11:10,13:10,15:10,17:10";
            this.Description = "自动抓取阳江市政府采购招标信息";
            this.SiteUrl = "http://yangjiang.gdgpo.com/queryMoreInfoList/channelCode/0005.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://yangjiang.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://yangjiang.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            msgType = "阳江市政府采购";
                            specType = "政府采购";
                            InviteInfo info = ToolDb.GenInviteInfo("广东省", "阳江市区", "", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, inviteType, specType, otherType, InfoUrl, HtmlTxt);
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://yangjiang.gdgpo.gov.cn" + fileAtag.Link;
