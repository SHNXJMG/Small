...
{
    public class InviteMzzfcg : WebSiteCrawller
    {
        public InviteMzzfcg()
            : base()
        {
            this.Group = "政府采购招标信息";
            this.Title = "梅州市政府采购招标信息";
            this.PlanTime = "9:31,11:31,13:31,15:31,17:31";
            this.Description = "自动抓取梅州市政府采购招标信息";
            this.SiteUrl = "http://meizhou.gdgpo.com/queryMoreInfoList/channelCode/0005.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://meizhou.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://meizhou.gdgpo.com" + aTag.Link; 
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://meizhou.gdgpo.gov.cn" + fileAtag.Link;
