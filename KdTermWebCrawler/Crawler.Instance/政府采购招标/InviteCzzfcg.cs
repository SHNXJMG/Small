...
{
    public class InviteCzzfcg : WebSiteCrawller
    {
        public InviteCzzfcg()
            : base()
        {
            this.Group = "政府采购招标信息";
            this.Title = "潮州市政府采购招标信息";
            this.PlanTime = "9:04,11:04,13:04,15:04,17:04";
            this.Description = "自动抓取潮州市政府采购招标信息";
            this.SiteUrl = "http://chaozhou.gdgpo.com/queryMoreInfoList/channelCode/0005.html";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    ...
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://chaozhou.gdgpo.com/queryMoreInfoList.do", nvc, Encoding.UTF8);
                    }
                    catch
                    {
                        continue;
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        InfoUrl = "http://chaozhou.gdgpo.com" + aTag.Link;
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            {
                                code = inviteCtx.GetRegexBegEnd("项目编号：", "）");
                                if (Encoding.Default.GetByteCount(code) > 50) code = string.Empty;
                            }
                            ...
                            msgType = "潮州市政府采购";
                            specType = "政府采购";
                            InviteInfo info = ToolDb.GenInviteInfo("广东省", "潮州市区", "", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, inviteType, specType, otherType, InfoUrl, HtmlTxt);
                                ...
                                for (int k = 0; k < fileNode.Count; k++)
                                {
                                    ...
                                    if (fileAtag.IsAtagAttach())
                                    {
                                        ...
                                        if (!fileLink.ToLower().Contains("http"))
                                            fileLink = "http://chaozhou.gdgpo.gov.cn" + fileAtag.Link;
