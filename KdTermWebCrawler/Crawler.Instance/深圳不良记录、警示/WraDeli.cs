...
{
    public class WraDeli:WebSiteCrawller
    {
        public WraDeli()
           : base()
       {
           this.Group = "警示信息";
           this.Title = "深圳市住房和建设局不良行为记录";
           this.Description = "自动抓取深圳市住房和建设局不良行为记录";
           this.PlanTime = "1 1:00";
           this.ExistCompareFields = "WarningName,WarningType,CorpType,DeliveryDate";
           this.MaxCount = 200;
           this.SiteUrl = "http://61.144.226.2:8001/web/cxda/hldjsAction.do?method=toXWJLList";
       }
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                if (nodeList != null && nodeList.Count > 0)
                {
                    ...
                    for (int j = 1; j < table.RowCount; j++)
                    {
                        ...
                        warningName = tr.Columns[1].ToNodePlainString();
                        corpType = tr.Columns[2].ToNodePlainString();
                        badInfo = tr.Columns[3].ToNodePlainString();
                        deliveryDate = tr.Columns[4].ToPlainTextString().GetDateRegex();
                        warningType = "不良行为记录";
                        msgType = "深圳市住房和建设局";
