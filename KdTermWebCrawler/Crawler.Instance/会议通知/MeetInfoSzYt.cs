...
{
    public class MeetInfoSzYt : WebSiteCrawller
    {
        public MeetInfoSzYt()
            : base()
        {
            this.Group = "会议信息";
            this.Title = "广东省深圳市政府采购盐田分中心";
            this.Description = "自动抓取广东省深圳市政府采购盐田分中心";
            ...
            this.SiteUrl = "http://yt.szzfcg.cn/";
        }
            ...
            if (nodeList != null && nodeList.Count > 0)
            {
                ...
                for (int i = 0; i < table.RowCount; i++)
                {
                    ...
                    if (tr.GetAttribute("valign") == "top")
                    {
                        ...
                        string temp = tr.Columns[1].ToPlainTextString().Replace("（2）", "").Replace("(2)", "").Replace("(二)", "").Replace("（二）", "");
                        ...
                        if ((string.IsNullOrEmpty(place) || !place.IsChina()) && !string.IsNullOrEmpty(meetTime) && !string.IsNullOrEmpty(prjName))
                        {
                            place = meetstr.Replace(prjName, "").Replace(meetTime, "").Replace(" ", "");
                        }
                        ...
                        MeetInfo info = ToolDb.GenMeetInfo("广东省", "深圳政府采购", string.Empty, string.Empty, prjName, place, meetName, meetTime,
                              string.Empty, "深圳市政府采购盐田分中心", SiteUrl, code, builUnit, string.Empty, string.Empty);
                        list.Add(info);
                        ...
                        if (!crawlAll && list.Count >= this.MaxCount)
                        {
                            ...
                            string sqlwhere = " where City='深圳政府采购' and InfoSource='深圳市政府采购盐田分中心' and BeginDate>='" + bDate + "' and BeginDate<='" + eDate + "'";
