...
{
    public class MeetInfoSzZFCG : WebSiteCrawller
    {
        public MeetInfoSzZFCG()
            : base()
        {
            this.Group = "会议信息";
            this.Title = "广东省深圳市政府采购";
            this.Description = "自动抓取广东省深圳市政府采购";
            ...
            this.SiteUrl = "http://www.szzfcg.cn/portal/openBid.jsp";
        }
            ...
            if (nodeList != null && nodeList.Count > 0)
            {
                ...
                for (int i = 0; i < table.RowCount; i++)
                {
                    ...
                    if (tr.GetAttribute("valign") == "top")
                    {
                        ...
                        {
                            meetstr = temp.Replace(code, "").Replace("（", "").Replace("(", "").Replace("）", "").Replace(")", "");
                        }
                        else
                            meetstr = temp;
                        string[] str = meetstr.Split(' ');
                        prjName = str[0];
                        if (String.IsNullOrEmpty(prjName) && str.Length > 1)
                            prjName = str[1];
                        meetName = "开标会";
                        ...
                        if (!string.IsNullOrEmpty(meetTime) && !string.IsNullOrEmpty(prjName))
                        ...
                        MeetInfo info = ToolDb.GenMeetInfo("广东省", "深圳政府采购", string.Empty, string.Empty, prjName, place, meetName, meetTime,
                              string.Empty, "深圳市政府采购", SiteUrl, code, string.Empty, string.Empty, string.Empty);
                        list.Add(info);
                        ...
                        if (!crawlAll && list.Count >= this.MaxCount)
                        {
                            ...
                            string sqlwhere = " where City='深圳政府采购' and InfoSource='深圳市政府采购' and BeginDate>='" + bDate + "' and BeginDate<='" + eDate + "'";
