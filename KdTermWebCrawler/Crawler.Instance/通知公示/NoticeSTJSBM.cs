...
{
    public class NoticeSTJSBM : WebSiteCrawller
    {
        public NoticeSTJSBM()
            : base()
        {
            this.Group = "通知公示";
            this.Title = "广东省汕头建设网报名情况";
            this.PlanTime = "9:20,11:20,14:20,17:20";
            this.Description = "自动抓取广东省汕头建设网报名情况";
            this.ExistCompareFields = "InfoUrl";
            this.SiteUrl = "http://www.stjs.gov.cn/zbtb/zhaobiao_baoming.asp";
            this.MaxCount = 600;
        }
                ...
                if (nodeList != null && nodeList.Count > 2)
                {
                    ...
                    for (int j = 0; j < table.RowCount; j++)
                    {
                        ...
                        if (dtlList != null && dtlList.Count > 0)
                        {
                            ...
                            NodeList tabList = parser.ExtractAllNodesThatMatch(new TagNameFilter("tr"));
                            string ctx = string.Empty;
                            if (tabList != null && tabList.Count > 0)
                            {
                                for (int k = 0; k < tabList.Count; k++)
                                {
                                    TableRow tabTr = tabList[k] as TableRow;
                                    for (int d = 0; d < tabTr.ColumnCount; d++)
                                    {
                                        if ((d + 1) % 2 == 0)
                                        {
                                            ctx += tabTr.Columns[d].ToNodePlainString()+"\r\n";
                                        }
                                        else
                                        {
                                            ctx += tabTr.Columns[d].ToNodePlainString()+"：";
