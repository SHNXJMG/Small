...
{
    public class NoticeHZJSHD : WebSiteCrawller
    {
        public NoticeHZJSHD()
            : base()
        {
            this.Group = "通知公示";
            this.Title = "广东省惠州市建设工程交易中心（惠东县）资格预审";
            this.PlanTime = "9:20,11:20,14:20,17:20";
            this.Description = "自动抓取广东省惠州市建设工程交易中心（惠东县）资格预审";
            this.ExistCompareFields = "InfoUrl";
            this.SiteUrl = "http://www.ebc.huizhou.gov.cn/index/showList/000000000003/000000000430";
            this.MaxCount = 500;
        }
                ...
                if (nodeList != null && nodeList.Count > 0)
                {
                    ...
                    for (int j = 1; j < table.RowCount - 1; j++)
                    {
                        ...
                        if (dtlList != null && dtlList.Count > 0)
                        {
                            ...
                            if (string.IsNullOrEmpty(PublistTime))
                                PublistTime = endDate;
                            NoticeInfo info = ToolDb.GenNoticeInfo("广东省", "惠州市区", "惠东县", string.Empty, InfoTitle, InfoType, InfoCtx, PublistTime, string.Empty, MsgTypeCosnt.HuiZhouMsgType, InfoUrl, prjCode, buildUnit, string.Empty, string.Empty, string.Empty, string.Empty, htmlTxt);
                            list.Add(info);
