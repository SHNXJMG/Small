...
{
    public class BidFuYong : WebSiteCrawller
    {
        public BidFuYong()
            : base()
        {
            this.Group = "街道办中标信息";
            this.Title = "广东省深圳市福永街道办事处";
            this.Description = "自动抓取广东省深圳市福永街道办事处中标信息";
            this.PlanTime = "9:20,13:51";
            this.SiteUrl = "http://fyjdb.baoan.gov.cn/xxgk_11867/ywxx/zbcg/zbxxgg/";
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://fyjdb.baoan.gov.cn/xxgk_11867/ywxx/zbcg/zbxxgg/index_" + (i - 1).ToString() + ".html", Encoding.UTF8);
                    }
                    catch { continue; }
                }
                ...
                if (viewList != null && viewList.Count > 0)
                {
                    for (int j = 0; j < viewList.Count; j++)
                    {
                        ...
                        try
                        {
                            ...
                            InfoUrl = "http://fyjdb.baoan.gov.cn/xxgk_11867/ywxx/zbcg/zbxxgg/" + aTag.Link.Replace("../", "").Replace("./", "");
                        }
                        catch { continue; }
                        ...
                        if (dtl != null && dtl.Count > 0)
                        {
                                ...
                                for (int k = 0; k < table.RowCount; k++)
                                {
                                    for (int c = 0; c < table.Rows[k].ColumnCount; c++)
                                    {
                                        if (table.RowCount > 1 && k == 0)
                                        {
                                            ...
                                            bidCtx += strCtx + "：" + table.Rows[k + 1].Columns[c].ToPlainTextString().Replace("&nbsp;", "").Replace(" ", "")+"\r\n";
                                        }
                                    }
                                    break;
                                }
                            ...
                            Regex regBidUnit = new Regex(@"(第一候选人|中标候选人|中标单位|中标人|中标方|中标供应商)(：|:)[^\r\n]+\r\n");
                            bidUnit = regBidUnit.Match(bidCtx.Replace(" ", "")).Value.Replace("中标供应商", "").Replace("中标候选人", "").Replace("第一候选人", "").Replace("中标单位", "").Replace("中标人", "").Replace("中标方", "").Replace("：", "").Replace(":", "").Trim();
                            ...
                            {
                                buildUnit = "深圳市宝安区福永街道办事处";
                            }
                            msgType = "深圳市宝安区福永街道办事处";
                            specType = "建设工程";
                            bidType = "小型工程"; 
