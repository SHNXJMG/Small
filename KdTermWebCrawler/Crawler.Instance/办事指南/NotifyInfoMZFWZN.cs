...
{
    public class NotifyInfoMZFWZN : WebSiteCrawller
    {
        public NotifyInfoMZFWZN()
            : base()
        {
            this.Group = "办事指南";
            this.Title = "广东省梅州市建设工程交易中心服务指南";
            this.Description = "自动抓取广东省梅州市建设工程交易中心服务指南";
            this.PlanTime = "21:26";
            this.SiteUrl = "http://market.meizhou.gov.cn/deptWebsiteAction.do?action=secondIndex&deptId=1925&issueTypeCode=009007&issueTypeName=服务指南";
            this.MaxCount = 500;
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://market.meizhou.gov.cn/deptWebsiteAction.do?action=secondIndex&deptId=1925&issueTypeCode=009007&issueTypeName=服务指南&pageNum=" + i.ToString(), Encoding.Default).GetJsString();
                    }
                    catch { continue; }
                }
