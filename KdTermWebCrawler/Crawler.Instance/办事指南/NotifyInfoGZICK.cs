...
{
    public class NotifyInfoGZICK : WebSiteCrawller
    {
        public NotifyInfoGZICK()
            : base()
        {
            this.Group = "办事指南";
            this.Title = "广东省广州市建设工程交易中心（IC卡指引）办事指南";
            this.Description = "自动抓取广东省广州市建设工程交易中心（IC卡指引）办事指南";
            this.PlanTime = "22:04";
            this.SiteUrl = "http://www.gzzb.gd.cn/cms/wz/view/fwzn/ick_infolist.jsp?siteId=1&channelId=49";
            this.MaxCount = 500;
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            if (pageList != null && pageList.Count > 0)
            {
                try
                {
                    string temp = pageList.GetATag(pageList.Count - 1).Link + "kdxx";
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                NodeList nodeList = parser.ExtractAllNodesThatMatch(new AndFilter(new HasParentFilter(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("class", "bszn_right_table")), true), new TagNameFilter("table")));
                            ...
                            if (ToolDb.SaveEntity(info, this.ExistCompareFields))
                            {
                                ...
                                if (imgList != null && imgList.Count > 0)
                                {
                                    for (int img = 0; img < imgList.Count; img++)
                                    {
                                        ...
                                        try
                                        {
                                            BaseAttach obj = ToolHtml.GetBaseAttach(imgTag.GetAttribute("src"), headName, info.Id);
                                            if (obj != null)
