...
{
    public class InviteDgQiaoTou : WebSiteCrawller
    {
        public InviteDgQiaoTou()
            : base()
        {
            this.Group = "街道办招标信息";
            this.Title = "广东省东莞桥头镇政府信息招标公告";
            this.Description = "自动抓取东莞桥头镇政府信息招标公告";
            this.PlanTime = "9:18";
            this.SiteUrl = "http://www.qiaotou.gov.cn/publicfiles/business/htmlfiles/qiaot/s32857/index.htm";
            this.MaxCount = 150;
        }
            ...
            if (pageNode != null && pageNode.Count > 0)
            {
                for (int i = 0; i < pageNode.Count; i++)
                {
                   ...
                   CreateTime = string.Empty, msgType = string.Empty, otherType = string.Empty, HtmlTxt = string.Empty, area = string.Empty;
                    ...
                    InfoUrl = "http://www.qiaotou.gov.cn/publicfiles/business/htmlfiles/" + urlNode[0].ToPlainTextString();
                    ...
                    NodeList titleNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("td"), new HasAttributeFilter("class","concenttitle")));
                    ...
                    if (dtlNode != null && dtlNode.Count > 0)
                    {
                        ...
                        InviteInfo info = ToolDb.GenInviteInfo("广东省", "东莞市区", "桥头镇", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, inviteType, specType, otherType, InfoUrl, HtmlTxt);
                        list.Add(info);
                            ...
                            for (int k = 0; k < aNode.Count; k++)
                            {
                                ...
                                if (a.IsAtagAttach())
                                {
                                    ...
                                    if (a.Link.ToLower().Contains("http"))
                                        ...
                                        link = "http://www.qiaotou.gov.cn/" + a.Link;
