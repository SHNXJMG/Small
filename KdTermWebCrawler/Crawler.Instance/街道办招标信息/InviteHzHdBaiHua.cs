...
{
    public class InviteHzHdBaiHua : WebSiteCrawller
    {
        public InviteHzHdBaiHua()
            : base()
        {
            this.Group = "街道办招标信息";
            this.Title = "广东省惠州市惠东县白花镇人民政府招标、中标公告";
            this.Description = "自动抓取广东省惠州市惠东县白花镇人民政府招标、中标公告";
            this.PlanTime = "9:18";
            this.SiteUrl = "http://baihua.huidong.gov.cn/bigClassdeta.asp?typeid=65&BigClassid=287";
            this.MaxCount = 100;
        }
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                if (viewList != null && viewList.Count > 0)
                {
                    for (int j = 0; j < viewList.Count; j++)
                    {
                        ...
                        string tempName = aTag.LinkText.Trim().GetReplace(" ");
                        beginDate = table.ToNodePlainString().GetDateRegex();
                        InfoUrl = "http://baihua.huidong.gov.cn/" + aTag.Link;
                        ...
                        if (dtl != null && dtl.Count > 0)
                        {
                            ...
                            if (tempName.Contains("中标") || tempName.Contains("成交") || tempName.Contains("结果"))
                            {
                          ...
                          prjMgr = string.Empty, otherType = string.Empty;
                                ...
                                prjName = bidCtx.GetRegex("工程名称,项目名称,工程监理名称");
                                    ...
                                    for (int k = 0; k < aNode.Count; k++)
                                    {
                                        ...
                                        if (a.IsAtagAttach())
                                        {
                                            ...
                                            if (a.Link.ToLower().Contains("http"))
                                                ...
                                                link = "http://baihua.huidong.gov.cn/" + a.Link;
                       ...
                       CreateTime = string.Empty, msgType = string.Empty, otherType = string.Empty;
                                ...
                                prjName = inviteCtx.GetRegex("工程名称,项目名称,工程监理名称");
                                if (string.IsNullOrEmpty(prjName))
                                    prjName = tempName;
