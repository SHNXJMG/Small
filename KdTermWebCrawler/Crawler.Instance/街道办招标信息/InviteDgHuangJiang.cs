...
{
    public class InviteDgHuangJiang : WebSiteCrawller
    {
        public InviteDgHuangJiang()
            : base()
        {
            this.Group = "街道办招标信息";
            this.Title = "广东省东莞黄江镇政府信息招标公告";
            this.Description = "自动抓取东莞黄江镇政府信息招标公告";
            this.PlanTime = "9:18";
            this.SiteUrl = "http://hj.dg.gov.cn/business/htmlfiles/dghj/zbxx/list.htm";
            this.MaxCount = 150;
        }
            ...
            if (pageNode != null && pageNode.Count > 0)
            {
                for (int i = 0; i < pageNode.Count; i++)
                {
                   ...
                   CreateTime = string.Empty, msgType = string.Empty, otherType = string.Empty, HtmlTxt = string.Empty, area = string.Empty;
                    ...
                    InfoUrl = "http://hj.dg.gov.cn/business/htmlfiles/" + urlNode[0].ToPlainTextString();
                    ...
                    parser.Reset();
                    NodeList dtlNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("id", "zoomcon")));
                    if (dtlNode != null && dtlNode.Count > 0)
                    {
                        ...
                        InviteInfo info = ToolDb.GenInviteInfo("广东省", "东莞市区", "黄江镇", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, inviteType, specType, otherType, InfoUrl, HtmlTxt);
                        list.Add(info);
