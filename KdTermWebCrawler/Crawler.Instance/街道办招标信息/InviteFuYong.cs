...
{
    public class InviteFuYong : WebSiteCrawller
    {
        public InviteFuYong()
            : base()
        {
            ...
            this.Description = "自动抓取广东省深圳市福永街道办事处招标信息";
            this.PlanTime = "9:18,13:49";
            this.SiteUrl = "http://fyjdb.baoan.gov.cn/xxgk_11867/ywxx/zbcg/zbxxgs/";
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://fyjdb.baoan.gov.cn/xxgk_11867/ywxx/zbcg/zbxxgs/index_" + (i - 1).ToString() + ".html", Encoding.UTF8);
                    }
                    catch { continue; }
                }
                ...
                if (viewList != null && viewList.Count > 0)
                {
                    for (int j = 0; j < viewList.Count; j++)
                    {
                        ...
                        try
                        {
                            ...
                            InfoUrl = "http://fyjdb.baoan.gov.cn/xxgk_11867/ywxx/zbcg/zbxxgs/" + aTag.Link.Replace("../", "").Replace("./", "");
                        }
                        catch { continue; }
