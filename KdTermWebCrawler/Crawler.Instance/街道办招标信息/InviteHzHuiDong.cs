...
{
    public class InviteHzHuiDong : WebSiteCrawller
    {
        public InviteHzHuiDong()
            : base()
        {
            this.Group = "街道办招标信息";
            this.Title = "广东省惠州市惠东县人民政府平山街道办事处招标、中标公告";
            this.Description = "自动抓取广东省惠州市惠东县人民政府平山街道办事处招标、中标公告";
            this.PlanTime = "9:18";
            this.SiteUrl = "http://www.psjd.gov.cn/Channel14.asp";
            this.MaxCount = 100;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                NodeList viewList = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("class", "art_title")));
                if (viewList != null && viewList.Count > 0)
                {
                    for (int j = 0; j < viewList.Count; j++)
                    {
                        ...
                        prjName = aTag.LinkText.Trim().GetReplace("&nbsp;"); ;
                        InfoUrl = "http://www.psjd.gov.cn/" + aTag.Link;
                        ...
                        NodeList dtl = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("class", "incontentdiv")));
                        if (dtl != null && dtl.Count > 0)
                        {
                            HtmlTxt = dtl.AsHtml();

                            parser = new Parser(new Lexer(HtmlTxt));
                            NodeList bottomNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("id", "bottomnav")));
                            if (bottomNode != null && bottomNode.Count > 0)
                            {
                                string htmlBotm = bottomNode.AsHtml();
                                HtmlTxt = HtmlTxt.Replace(htmlBotm, "");
                            }
                            parser = new Parser(new Lexer(HtmlTxt));
                            NodeList alignNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("align", "right")));
                            if (alignNode != null && alignNode.Count > 0)
                            {
                                string htmlBotm = alignNode.AsHtml();
                                HtmlTxt = HtmlTxt.Replace(htmlBotm, "");
                            }
                            parser = new Parser(new Lexer(HtmlTxt));
                            NodeList stitNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("p"), new HasAttributeFilter("class", "stit")));
                            if (stitNode != null && stitNode.Count > 0)
                            {
                                string htmlBotm = stitNode.AsHtml();
                                HtmlTxt = HtmlTxt.Replace(htmlBotm, "");
                                beginDate = stitNode[0].ToPlainTextString().GetDateRegex("yyyy/MM/dd");
                            }

                            string ctx = HtmlTxt.ToLower().GetReplace("</p>,</br>,<br>", "\r\n").ToCtxString();
                            if (string.IsNullOrEmpty(beginDate))
                                beginDate = Regex.Match(ctx.Trim(), @"\d{4}年\d{1,2}月\d{1,2}日", RegexOptions.RightToLeft).Value;
                                    ...
                                    for (int k = 0; k < aNode.Count; k++)
                                    {
                                        ...
                                        if (a.IsAtagAttach())
                                        {
                                            ...
                                            if (a.Link.ToLower().Contains("http"))
                                                ...
                                                link = "http://www.psjd.gov.cn/" + a.Link;
