...
{
    public class InviteSzLongHua:WebSiteCrawller
    {
        public InviteSzLongHua()
            : base()
        {
            this.Group = "街道办招标信息";
            this.Title = "深圳市龙华新区公告资源交易中心建设工程招标公告";
            this.Description = "自动抓取深圳市龙华新区公告资源交易中心建设工程招标公告";
            this.PlanTime = "9:22,13:53";
            this.SiteUrl = "http://www.szlhxq.gov.cn/lhxinqu/zdlyxxgkzl/ggzyjy/jsgczbgg/index.html";
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://www.szlhxq.gov.cn/lhxinqu/zdlyxxgkzl/ggzyjy/jsgczbgg/9614-" + i + ".html", Encoding.UTF8);
                    }
                    catch { continue; }
                }
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            specType = "建设工程";
                            InviteInfo info = ToolDb.GenInviteInfo("广东省", "深圳区及街道工程", "龙华新区", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, inviteType, specType, otherType, InfoUrl, HtmlTxt);
                            list.Add(info);
