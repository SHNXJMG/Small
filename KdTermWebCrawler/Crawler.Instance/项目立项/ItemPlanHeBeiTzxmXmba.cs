...
{
    public class ItemPlanHeBeiTzxmXmba : WebSiteCrawller
    {
        public ItemPlanHeBeiTzxmXmba() :
            base()
        {
            this.Group = "项目立项";
            this.Title = "河北省投资项目管理系统项目立项（备案项目）";
            this.PlanTime = "9:04,11:04,13:04,15:04,17:04";
            this.Description = "自动抓取河北省投资项目管理系统项目立项（备案项目）";
            this.SiteUrl = "http://hebpi.net:80/portal/ShowMoreProjectAction.do?method=BaProListPage&page=1&rp=20";
            this.MaxCount = 500;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://hebpi.net:80/portal/ShowMoreProjectAction.do?method=BaProListPage&page=" + i + "&rp=20");
                    }
                    catch { continue; }
                }
