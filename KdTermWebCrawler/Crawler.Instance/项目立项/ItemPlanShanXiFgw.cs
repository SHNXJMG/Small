...
{
    public class ItemPlanShanXiFgw : WebSiteCrawller
    {
        public ItemPlanShanXiFgw()
            : base()
        {
            this.Group = "项目立项";
            this.Title = "山西省发展和改革委员会项目立项";
            this.Description = "自动抓取山西省发展和改革委员会项目立项";
            this.PlanTime = "8:52,9:42,10:32,11:32,13:42,15:02,16:32";
            this.ExistCompareFields = "InfoUrl";
            this.SiteUrl = "http://www.sxdrc.gov.cn/xxlm/lxsp/";
            this.MaxCount = 400;
        }
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            if (pageNode != null && pageNode.Count > 0)
            {
                try
                {
                    string temp = pageNode.AsString().GetRegexBegEnd("HTML", ",").GetReplace("(");
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                NodeList listNode = parser.ExtractAllNodesThatMatch(new AndFilter(new HasParentFilter(new AndFilter(new TagNameFilter("ul"), new HasAttributeFilter("class", "document_list")), true), new TagNameFilter("li")));
                if (listNode != null && listNode.Count > 0)
                {
                    for (int j = 0; j < listNode.Count; j++)
                    {
                        ...
                        NodeList dtlNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("id", "i_content")));
                        if (dtlNode != null && dtlNode.Count > 0)
                        {
                            ...
                            PlanType = "审批核准备案";
                            MsgType = "山西省发展和改革委员会";
                            ItemPlan info = ToolDb.GenItemPlan("山西省", "山西省及地市", "", ItemCode, ItemName, ItemAddress, BuildUnit, BuildNature, TotalInvest, PlanInvest, IssuedPlan, InvestSource, ApprovalUnit, ApprovalDate, ApprovalCode, MsgUnit, PlanDate, PlanType, PlanBeginDate, PlanEndDate, CtxHtml, ItemCtx, ItemContent, MsgType, InfoUrl);
                            ...
                            if (aNode != null && aNode.Count > 0)
                            {
                                for (int k = 0; k < aNode.Count; k++)
                                {
                                    ...
                                    if (a.IsAtagAttach())
                                    {
                                        string temp = InfoUrl.Remove(InfoUrl.LastIndexOf("/"));
                                        string link = string.Empty;
                                        if (a.Link.ToLower().Contains("http"))
                                            link = a.Link;
                                        else
                                            link = temp +"/"+ a.Link.GetReplace("../,./");
