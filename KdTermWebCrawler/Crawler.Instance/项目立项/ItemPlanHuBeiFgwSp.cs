...
{
    public class ItemPlanHuBeiFgwSp:WebSiteCrawller
    {
        public ItemPlanHuBeiFgwSp()
            : base()
        {
            this.Group = "项目立项";
            this.Title = "湖北省发展和改革委员会项目审批信息";
            this.PlanTime = "9:04,11:04,13:04,15:04,17:04";
            this.Description = "自动抓取湖北省发展和改革委员会项目审批信息";
            this.SiteUrl = "http://www.hbfgw.gov.cn/hqfw/xmgg/xmspgg/index.shtml";
            this.MaxCount = 2000;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://www.hbfgw.gov.cn/hqfw/xmgg/xmspgg/index_" + (i - 1).ToString() + ".shtml");
                    }
                    catch { continue; }
                }
                ...
                if (listNode != null && listNode.Count > 0)
                {
                    ...
                    for (int j = 1; j < table.RowCount; j++)
                    {
                        ...
                        ItemName = aTag.LinkText.GetReplace("省发改委批复,省发改委核准");
                        ApprovalUnit = tr.Columns[2].ToNodePlainString();
                        PlanDate = tr.Columns[3].ToPlainTextString().GetDateRegex();
                        InfoUrl = "http://www.hbfgw.gov.cn/hqfw/xmgg/xmspgg/" + aTag.Link.GetReplace("../,./");
