...
{
    public class ItemPlanHuiNanBA : WebSiteCrawller
    {
        public ItemPlanHuiNanBA()
            : base()
        {
            this.Group = "项目立项";
            this.Title = "湖南省发展和改革委员会项目立项备案信息";
            this.PlanTime = "9:04,11:04,13:04,15:04,17:04";
            this.Description = "自动抓取湖南省发展和改革委员会项目立项备案信息";
            this.SiteUrl = "http://www.hnfgw.gov.cn/xxgk/spgbba/Index.html";
            this.MaxCount = 1200;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            if (pageNode != null && pageNode.Count > 0)
            {
                try
                {
                    ...
                    pageInt = int.Parse(temp.GetReplace(new string[] { "/xxgk/spgbba/Index_", ".html" }));
                }
                catch { }
            }
            for (int i = 1; i <= pageInt; i++)
            {
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://www.hnfgw.gov.cn/xxgk/spgbba/Index_" + i + ".html");
                    }
                    catch { continue; }
                }
