...
{
    public class NotifyInfoMZJSTZGG : WebSiteCrawller
    {
        public NotifyInfoMZJSTZGG()
            : base()
        {
            this.Group = "通知公告";
            this.Title = "广东省梅州市建设工程交易中心（市直）通知公告";
            this.Description = "自动抓取广东省梅州市建设工程交易中心（市直）通知公告";
            this.PlanTime = "22:00";
            this.SiteUrl = "http://market.meizhou.gov.cn/deptWebsiteAction.do?action=secondIndex&deptId=1925&issueTypeCode=009006001&issueTypeName=通知公告&showSubNodeflag=1";
            this.MaxCount = 500;
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            { 
                if (i > 1)
                {
                    try
                    {
                        html = this.ToolWebSite.GetHtmlByUrl("http://market.meizhou.gov.cn/deptWebsiteAction.do?action=secondIndex&deptId=1925&showSubNodeflag=1&issueTypeCode=009006001&issueTypeName=通知公告&pageNum=" + i.ToString(), Encoding.Default).GetJsString();
                    }
                    catch { continue; }
                }
