...
{
    public class NotifyInfoSzJsLGtz : WebSiteCrawller
    {
        public NotifyInfoSzJsLGtz()
            : base()
        {
            this.Group = "通知公告";
            this.Title = "广东省深圳市建设工程交易中心龙岗分中心通知公告";
            this.Description = "自动抓取广东省深圳市建设工程交易中心龙岗分中心通知公告";
            this.PlanTime = "21:05";
            this.SiteUrl = "http://jyzx.cb.gov.cn/LGjyzxWeb/SiteManage/PublicNotice.aspx?MenuName=PublicNotice&ModeId=1&ItemId=29A78FD5F48D445093BBFD77CEAB2ADC&ItemName=%e9%80%9a%e7%9f%a5%e5%85%ac%e5%91%8a&clearpaging=true";
        }
        ...
        private void AddBaseFile(string infoUrl, string strFileName, NotifyInfo info)
        {
            ...
            try
            {
                ...
                fs.Close();
                ns.Close();
                //FileInfo fi = new FileInfo(strFileUrl + strFile + path);
                if (length > 1024)
                {
