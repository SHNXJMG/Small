...
{
    public class NotifyInfoDGJSZXGG : WebSiteCrawller
    {
        public NotifyInfoDGJSZXGG()
            : base()
        {
            this.Group = "通知公告";
            this.Title = "广东省东莞市建设工程交易中心中心公告";
            this.Description = "自动抓取广东省东莞市建设工程交易中心中心公告";
            this.PlanTime = "21:55";
            this.SiteUrl = "http://www.dgzb.com.cn/DGJYWEB/SiteManage/Notify_List.aspx?ModeId=1";
            this.MaxCount = 500;
        }
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                if (nodeList != null && nodeList.Count > 0)
                {
                    ...
                    for (int j = 1; j < table.RowCount; j++)
                    {
                        ...
                        NodeList noList = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("div"), new HasAttributeFilter("id", "ctl00_cph_context_span_MetContent")));
                        if (noList != null && noList.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    parser = new Parser(new Lexer(htldtl));
                                    NodeList attachList = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("table"), new HasAttributeFilter("id", "ctl00_cph_context_DownLoadFiles1_GridView2")));
