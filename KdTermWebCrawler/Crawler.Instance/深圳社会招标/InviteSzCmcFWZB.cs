...
{
    public class InviteSzCmcFWZB : WebSiteCrawller
    {
        public InviteSzCmcFWZB()
            : base()
        {
            this.Group = "代理机构招标信息";
            this.Title = "中国机械进出口（集团）有限公司（服务招标）";
            this.PlanTime = "9:15,11:15,13:45,15:45,17:45";
            this.Description = "自动抓取中国机械进出口（集团）有限公司招标信息（服务招标）";
            this.SiteUrl = "http://zb.cmc.com.cn/TenderNotice/List.aspx?id=3";
            this.MaxCount = 50;
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            try
            {
                html = this.ToolWebSite.GetHtmlByUrl(this.ToolWebSite.UrlEncode(SiteUrl), Encoding.UTF8,ref cookie);
            }
            catch (Exception ex)
            {
            ...
            { 
                ...
                for (int i = 1; i <= pageInt; i++)
                {
                    ...
                    if (i > 1)
                    {
                        try
                        {
                            html = this.ToolWebSite.GetHtmlByUrl(this.ToolWebSite.UrlEncode(SiteUrl + "&page=" + i.ToString() ), Encoding.UTF8,ref cookie);
                        }
                        catch (Exception ex)
                        {
                        ...
                        for (int j = 1; j < table.RowCount; j++)
                          {
                            ...
                            try
                            {
                                ...
                                htmldetail = ToolHtml.GetHtmlByUrl(SiteUrl, InfoUrl);
