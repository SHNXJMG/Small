...
{
    public class CorpInfoSzKanChaSheJi : WebSiteCrawller
    {
        public CorpInfoSzKanChaSheJi()
            : base()
        {
            this.PlanTime = "1 23:30,8 23:30,16 23:30,24 23:30";//"1-28 2:00,4-28 2:00,7-28 2:00,10-28 2:00";
            this.Group = "企业信息";
            this.Title = "深圳市建设局企业信息（勘察设计）";
            this.Description = "自动抓取深圳市建设局企业信息（勘察设计）";
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            NameValueCollection nvc = ToolWeb.GetNameValueCollection(new string[] { "param", "corpType", "corp_name", "page" }, new string[] { "", "19", "", "1" });
            try
            {
            ...
            for (int i = 1; i <= pageInt; i++)
            {
                ...
                if (i > 1)
                {
                    nvc = ToolWeb.GetNameValueCollection(new string[] { "param", "corpType", "corp_name", "page" }, new string[] { "", "19", "", i.ToString() });
                    try
                    {
                        html = ToolWeb.GetHtmlByUrl(this.SiteUrl, nvc, Encoding.UTF8);
                    }
                ...
                if (nodeList != null && nodeList.Count > 0)
                {
                    ...
                    for (int j = 1; j < table.RowCount; j++)
                    {
                        ...
                        try
                        {
                            ...
                            cUrl = infoUrl + string.Format("?param={0}&corpType=19&orgCode={1}", postParams[0], CorpCode);
                        }
                        catch { continue; }
                        try
                        {
                        ...
                        CorpInfo info = ToolDb.GenCorpInfo(CorpName, CorpCode, CorpAddress, RegDate, RegFund, BusinessCode, BusinessType, LinkMan, LinkPhone, Fax, Email, CorpSite, "勘察设计企业", "广东省", "深圳市", "深圳市住房和建设局", cUrl, ISOQualNum, ISOEnvironNum, OffAdr);
                        ...
                        if (infoCount != -1 || !isDel)
                        {
                            if (ToolDb.SaveEntity(info, string.Empty))
                            {
                                ...
                                {
                                    if (qualCount != -1)
                                        AddCorpQual(info, postParams[0], "19");
                                    if (awardCount != -1)
                                        AddCorpAward(info, postParams[0], "19");
                                    //if (certCount != -1)
                                    //    AddCorpCert(info, htmldtl);
                                    //if (deviceCount != -1)
                                    //    AddCorpDevice(info, htmldtl);
                                    if (punishCount != -1)
                                        AddCorpPunish(info, postParams[0], "19");
                                    if (resultCount != -1)
                                        AddCorpResults(info, postParams[0], "19");
                                    if (seclicCount != -1)
                                        AddCorpSecLic(info, postParams[0], "19");
                                    if (seclicstaffCount != -1)
                                        AddCorpSecLicStaff(info, postParams[0], "19");
                                    if (tecstaffCount != -1)
                                        AddCorpTecStaff(info, postParams[0], "19");
                                    if (leaderCount != -1)
                                        AddCorpLeader(info, postParams[0], "19");
                                }
                                else
                                {
                                    AddCorpQual(info, postParams[0], "19");
