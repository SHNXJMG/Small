...
{
    public class SzCorpInfoJsXB : WebSiteCrawller
    {
        public SzCorpInfoJsXB()
            : base()
        {
            this.PlanTime = "1 23:30,8 23:30,16 23:30,24 23:30";//"1-28 2:00,4-28 2:00,7-28 2:00,10-28 2:00";
            this.Group = "企业信息";
            this.Title = "深圳市建设局企业信息（2014新版）";
            this.Description = "自动抓取深圳市建设局企业信息（2014新版）";
            this.ExistCompareFields = "CorpType,CorpName";
            this.MaxCount = 50000;
            this.SiteUrl = "http://portal.szjs.gov.cn:8888/publicShow/corpList.html";
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            //has.Add("勘察设计企业", "17");
            //has.Add("建筑业企业", "1");
            has.Add("设计与施工一体化企业", "24");
            has.Add("专项设计企业", "9");
            has.Add("施工图审查机构", "21");
            has.Add("监理企业", "4");
            has.Add("招标代理企业", "7");
            has.Add("造价咨询企业", "8");
            //has.Add("预拌混凝土和预拌砂浆企业", "18");
            //has.Add("检测单位", "13"); 
            //has.Add("房屋安全鉴定机构", ""); 
            ...
            foreach (string item in has.Keys)
            {
                ...
                nvc = ToolWeb.GetNameValueCollection(new string[] { "param", "corpType", "corp_name", "page" }, new string[] { "", has[item], "", "1" });
                try
                {
                ...
                for (int i = 1; i <= pageInt; i++)
                {
                    if (i > 1)
                    {
                        nvc = ToolWeb.GetNameValueCollection(new string[] { "param", "corpType", "corp_name", "page" }, new string[] { "", has[item], "", i.ToString() });
                        try
                        {
                            html = ToolWeb.GetHtmlByUrl(url, nvc, Encoding.UTF8);
                        }
                    ...
                    if (nodeList != null && nodeList.Count > 0)
                    {
                        ...
                        for (int j = 1; j < table.RowCount; j++)
                        {
                            ...
                            }, new string[] { postParams[0], "19", postParams[1] });
                                cUrl = infoUrl + string.Format("?param={0}&corpType={1}&orgCode={2}", postParams[0],has[item], CorpCode);
                            }
                            catch { continue; }
                            try
                            {
                            ...
                            CorpInfo info = ToolDb.GenCorpInfo(CorpName, CorpCode, CorpAddress, RegDate, RegFund, BusinessCode, BusinessType, LinkMan, LinkPhone, Fax, Email, CorpSite, item, "广东省", "深圳市", "深圳市住房和建设局", cUrl, ISOQualNum, ISOEnvironNum, OffAdr);
                            ...
                            if (infoCount != -1 || !isDel)
                            {
                                if (ToolDb.SaveEntity(info, string.Empty))
                                {
                                    ...
                                    {
                                        if (qualCount != -1)
                                            AddCorpQual(info, postParams[0], has[item]);
                                        if (awardCount != -1)
                                            AddCorpAward(info, postParams[0], has[item]);
                                        //if (certCount != -1)
                                        //    AddCorpCert(info, htmldtl);
                                        //if (deviceCount != -1)
                                        //    AddCorpDevice(info, htmldtl);
                                        if (punishCount != -1)
                                            AddCorpPunish(info, postParams[0], has[item]);
                                        if (resultCount != -1)
                                            AddCorpResults(info, postParams[0], has[item]);
                                        if (seclicCount != -1)
                                            AddCorpSecLic(info, postParams[0], has[item]);
                                        if (seclicstaffCount != -1)
                                            AddCorpSecLicStaff(info, postParams[0], has[item]);
                                        if (tecstaffCount != -1)
                                            AddCorpTecStaff(info, postParams[0], has[item]);
                                        if (leaderCount != -1)
                                            AddCorpLeader(info, postParams[0], has[item]);
                                    }
                                    else
                                    {
                                        AddCorpQual(info, postParams[0], has[item]);
