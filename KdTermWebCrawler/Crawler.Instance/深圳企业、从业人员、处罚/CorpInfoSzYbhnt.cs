...
{
    public class CorpInfoSzYbhnt : WebSiteCrawller
    {
        public CorpInfoSzYbhnt()
            : base()
        {
            this.PlanTime = "1 23:30,8 23:30,16 23:30,24 23:30";//"1-28 2:00,4-28 2:00,7-28 2:00,10-28 2:00";
            this.Group = "企业信息";
            this.Title = "深圳市建设局企业信息（预拌混凝土和预拌砂浆企业）";
            this.Description = "自动抓取深圳市建设局企业信息（预拌混凝土和预拌砂浆企业）";
            this.ExistCompareFields = "CorpType,CorpName";
            this.MaxCount = 50000;
            this.SiteUrl = "http://61.144.226.2:8001/web/enterprs/unitInfoAction.do?method=toList&certType=18";
        }
        ...
        protected override IList ExecuteCrawl(bool crawlAll)
        {
            ...
            try
            {
                htl = ToolWeb.GetHtmlByUrl(this.SiteUrl, Encoding.Default);
            }
            catch { return null; }
                ...
                if (nodeList != null && nodeList.Count > 0)
                {
                    ...
                    for (int j = 1; j < table.RowCount; j++)
                    {
                        ...
                        try
                        {
                            ...
                            url = temp.Split(',');
                            cUrl = "http://61.144.226.2:8001/web/enterprs/unitInfoAction.do?method=toView&qybh=" + url[0] + "&certType=18&orgcode=" + url[1];
                        ...
                        NodeList dtlNode =
                            dtlNode = parser.ExtractAllNodesThatMatch(new AndFilter(new TagNameFilter("table"), new HasAttributeFilter("class", "infoTableL")));
                        ...
                        CorpInfo info = ToolDb.GenCorpInfo(CorpName, CorpCode, CorpAddress, RegDate, RegFund, BusinessCode, BusinessType, LinkMan, LinkPhone, Fax, Email, CorpSite, "预拌混凝土和预拌砂浆企业", "广东省", "深圳市", "深圳市住房和建设局", cUrl, ISOQualNum, ISOEnvironNum, OffAdr);
                        ...
                        if (infoCount != -1 || !isDel)
                        {
                            if (ToolDb.SaveEntity(info, string.Empty))
                            {
                                ...
                                {
                                    if (qualCount != -1)
                                        AddCorpQual(info, htmldtl, ctxKc, url);
                                    if (awardCount != -1)
                                        AddCorpAward(info, htmldtl, ctxKc, url);
                                    if (certCount != -1)
                                        AddCorpCert(info, htmldtl, ctxKc, url, corpRz);
                                    if (deviceCount != -1)
                                        AddCorpDevice(info, htmldtl, ctxKc, url);
                                    if (punishCount != -1)
                                        AddCorpPunish(info, htmldtl, ctxKc, url);
                                    if (resultCount != -1)
                                        AddCorpResults(info, htmldtl, ctxKc, url);
                                    if (seclicCount != -1)
                                        AddCorpSecLic(info, htmldtl, ctxKc, url);
                                    if (seclicstaffCount != -1)
                                        AddCorpSecLicStaff(info, htmldtl, ctxKc, url);
                                    if (tecstaffCount != -1)
                                        AddCorpTecStaff(info, htmldtl, ctxKc, url);
                                    if (leaderCount != -1)
                                        AddCorpLeader(info, htmldtl, ctxKc, url);
                                }
                                else
                                {
                                    AddCorpQual(info, htmldtl, ctxKc, url);
        ...
        public void AddCorpLeader(CorpInfo info, string html, string ctxKc, string[] url)
        {
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpCert(CorpInfo info, string html, string ctxKc, string[] url, string infoStr)
        {
            Parser parser = new Parser(new Lexer(html));
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpTecStaff(CorpInfo info, string html, string ctxKc, string[] url)
        {
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpDevice(CorpInfo info, string html, string ctxKc, string[] url)
        {
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpResults(CorpInfo info, string html, string ctxKc, string[] url)
        {
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpAward(CorpInfo info, string html, string ctxKc, string[] url)
        {
            Parser parser = new Parser(new Lexer(html));
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpPunish(CorpInfo info, string html, string ctxKc, string[] url)
        {
            Parser parser = new Parser(new Lexer(html));
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpSecLic(CorpInfo info, string html, string ctxKc, string[] url)
        {
            Parser parser = new Parser(new Lexer(html));
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpSecLicStaff(CorpInfo info, string html, string ctxKc, string[] url)
        {
        ...
        /// <param name="info"></param>
        /// <param name="html"></param>
        protected void AddCorpQual(CorpInfo info, string html, string ctxKc, string[] url)
        {
            Parser parser = new Parser(new Lexer(html));
