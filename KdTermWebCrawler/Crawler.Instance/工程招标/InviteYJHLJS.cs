...
{
    public class InviteYJHLJS : WebSiteCrawller
    {
        public InviteYJHLJS()
            : base()  
        {
            this.Group = "招标信息";
            this.Title = "广东省阳江市海陵县建设工程招标信息";
            this.Description = "自动抓取广东省阳江市海陵县建设工程招标信息";
            this.PlanTime = "9:30,11:30,14:30,16:30,18:30";
            this.SiteUrl = "http://www.yjgcjy.cn/ZBList.aspx?ItemID=91&SiteId=25";
        }
                    ...
                    for (int j = 2; j < table.RowCount - 1; j++)
                    {
                        ...
                        if (!code.Contains("发包"))
                        {
                            ...
                            if (dtnode != null && dtnode.Count > 0)
                            {
                                ...
                                Regex regBegin = new Regex(@"(公告发布时间：|报名时间)[^\r\n]+[\r\n]{1}");
                                beginDate = regBegin.Match(inviteCtx).Value.Replace("公告发布时间：", "").Replace("报名时间", "").Replace(":", "").Replace("：", "").Trim();
                                string date = beginDate.Replace(" ", "").Trim();
                                Regex regDate = new Regex(@"\d{4}年\d{1,2}月\d{1,2}日");
                                Regex regDate1 = new Regex(@"\d{4} 年\d{1,2}月\d{1,2}日");
                                beginDate = regDate.Match(date).Value.Trim();
                                if (beginDate == "")
                                {
                                    beginDate = regDate1.Match(date).Value.Trim();
                                }
                                if (beginDate == "")
                                {
                                ...
                                inviteCtx = inviteCtx.Replace(" xml:namespace prefix = st1 ns = ", "").Trim();
                                InviteInfo info = ToolDb.GenInviteInfo("广东省", "阳江市区", "海陵区", string.Empty, code, prjName, prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType, bidType, specType, otherType, InfoUrl, HtmlTxt);
                                list.Add(info);
                                parserdetail.Reset();
                        ...
                        else
                        {
                            ...
                            if (dtnode != null && dtnode.Count > 0)
                            {
                                ...
                                {
                                    beginDate = beginDate.Replace("年", "-").Replace("月", "-").Replace("日", "");
                                }
                                msgType = "阳江市建设工程交易中心";
                                specType = "建设工程";
                                inviteCtx = inviteCtx.Replace("<?", "").Replace("xml:namespace prefix = o ns = ", "").Replace("urn:schemas-microsoft-com:office:office", "").Replace("/>", "").Trim();
                                ...
                                inviteCtx = inviteCtx.Replace(" xml:namespace prefix = st1 ns = ", "").Trim();
                                bidType = ToolHtml.GetInviteTypes(prjName);
                                InviteInfo info = ToolDb.GenInviteInfo("广东省", "阳江市区", "海陵区", string.Empty, code, prjName,
                                    prjAddress, buildUnit, beginDate, endDate, inviteCtx, remark, msgType,
                                    bidType, specType, otherType, InfoUrl, HtmlTxt);
                                list.Add(info);
                                if (!crawlAll && list.Count >= this.MaxCount) return list;
                            }
                        }
